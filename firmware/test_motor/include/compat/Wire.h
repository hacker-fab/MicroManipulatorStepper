...
public:
    TwoWire()
        : m_i2c(i2c0), m_sda(PICO_DEFAULT_I2C_SDA_PIN), m_scl(PICO_DEFAULT_I2C_SCL_PIN) {}
    ...
    size_t write(uint8_t data) {
        if (m_tx_len < sizeof(m_tx_buf)) { m_tx_buf[m_tx_len++] = data; return 1; }
        return 0;
    }
    ...
    int available() const { return m_rx_len - m_rx_idx; }
    int read() { if (m_rx_idx < m_rx_len) return m_rx_buf[m_rx_idx++]; return -1; }
